...
export default function AdminMomentsPage() {
  ...
  const handleDelete = async (id: string, filePath: string) => {
    ...
    if (dbError) {
      toast.error("Delete failed from database");
    } else {
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              <MomentsForm
                ...
                addMoment={(moment) => setMoments((prev) => [moment, ...prev])}
              />
            </DialogContent>
          </Dialog>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {moments.map((moment) => (
              <div
                key={moment.id}
                className="bg-white rounded-lg shadow p-4 relative flex flex-col"
              >
                ...
                <button
                  ...
                  onClick={() => handleDelete(moment.id, moment.file_path)}
                  title="Delete"
                >
